...
from dataset import Dataset_GRIDdataset

from evaluate import evaluate_GRID

from style_dubber import Style_dubber_model_loss_15_GRID
...
def main(args, configs):
    ...
    Loss = Style_dubber_model_loss_15_GRID(preprocess_config, model_config).to(device)
    ...
    while True:
        ...
        for batchs in loader:
            for batch in batchs:
                ...
                total_loss, item_losses = Loss(text, speakers, mel_target, pitches, energies, mel_lens, durations, ref_linguistics, mel_predictions, postnet_mel_predictions, log_duration_predictions, src_masks, mel_masks, ref_mel_masks, speaker_predicts, spk_embedding, AV_attn, src_len, face_lens, lip_masks)
                ...
                if step % val_step == 0 and step != 0:
                    ...
                    model.eval()
                    fusion_model.eval()
                    mel_loss_all_val = evaluate_GRID(model, fusion_model, step, configs, val_log_path, val_logger, vocoder)
...
if __name__ == "__main__":
    ...
    aim = "./ModelConfig_GRID"
    parser.add_argument("--restore_step", type=int, default=0)
    parser.add_argument(
        "-t", "--train_config", type=str,
        default="config/GRID/train.yaml"
    )
    args = parser.parse_args()
    
    # load styledubber model_config and preprocess_config. (see "./ModelConfig_GRID/model_config/GRID/config_all.txt")
    with open(aim+'/model_config/GRID/config_all.txt', 'r') as file:
    ...
    # load styledubber train_config.  (see "./config/GRID/train.yaml")
