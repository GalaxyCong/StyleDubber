...
from dataset import Dataset_GRIDdataset_Setting1
...
def evaluate_test_allinference(model, fusion_model, step, configs, vocoder, val_samples_path):
    ...
    val_set = "val.txt"
    dataset = Dataset_GRIDdataset_Setting1(
...
if __name__ == "__main__":
    ...
    aim = "./outout_GRID"
    parser.add_argument("--restore_step", type=int, default=72500)
    parser.add_argument('--save_path', default='0_CR_Setting1GRID/{}'.format(aim.split('/')[-1]))
    
    args = parser.parse_args()
    val_samples_path = os.path.join(args.save_path, 'Setting1__Grid_Who_{}_Wav'.format(args.restore_step))
